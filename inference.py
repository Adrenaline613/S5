...
from model.spindle_feature import spindle_feature
...
def main():
    ...
    all_inputs = np.concatenate(all_inputs, axis=0).squeeze(1)
    all_preds = np.concatenate(all_preds, axis=0)

    # Get spindle features
    all_pred_sp_features = {
        "Duration": [],
        "Amplitude": [],
        "Frequency": [],
        "Oscillations": [],
    ...
    for input, pred in zip(all_inputs, all_preds):
        pred_sp_features = spindle_feature(input.astype(np.float64), pred, 100)
        for key in all_pred_sp_features.keys():
            all_pred_sp_features[key].extend(pred_sp_features[key])

    print(all_pred_sp_features)
